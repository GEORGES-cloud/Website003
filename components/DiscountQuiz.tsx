...
export default function DiscountQuiz({ locale }: { locale: string }) {
  ...
  const inputCls =
    'w-full border border-line bg-white px-5 py-3.5 font-sans text-[15px] text-ink placeholder:text-muted/60 focus:outline-none focus:border-flamingo-deep transition-colors';
            ...
            <div className="px-7 py-10 sm:px-10 sm:py-12 text-center">
              <motion.div key={step} initial={panelInit} animate={panelShown} transition={{ duration: reduce ? 0.2 : 0.4, ease: EASE }}>
                ...
                {step === 3 && (
                  <>
                    ...
                    <p className="inline-block font-sans font-semibold text-2xl sm:text-3xl tracking-[0.18em] text-ink border border-dashed border-flamingo/40 bg-flamingo-tint px-8 py-4 mb-5">
                      {CODE}
                    </p>
              ...
              {step < 3 && (
                <a
                  ...
                  rel="noopener noreferrer"
                  onClick={dismiss}
                  className="inline-flex items-center gap-2 font-sans text-[12px] font-semibold uppercase tracking-wide2 text-flamingo-deep hover:text-ink transition-colors mt-8"
                >
