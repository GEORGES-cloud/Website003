  ...
  return (
    <section className="py-24 md:py-32 bg-sand">
      <div className="max-w-[1480px] mx-auto px-6 md:px-10">
        <ScrollReveal>
          <h2 className="display text-ink text-center mb-14" style={{ fontSize: 'clamp(2.25rem, 4.5vw, 4rem)' }}>
            {title}
          </h2>
        </ScrollReveal>

        <div className="grid grid-cols-2 md:grid-cols-3 grid-rows-3 gap-3 md:gap-4 h-[560px] md:h-[720px]">
          {CELLS.map((cell, i) => (
            <motion.div
              key={i}
              className={`relative overflow-hidden rounded-2xl ${cell.row} group`}
