            ...
            {EXPERIENCES.map(({ key, image }, i) => (
              <ScrollReveal key={key} delay={(i % 2) * 0.1}>
                <article className="group">
                  <div className="relative aspect-[3/2] overflow-hidden rounded-2xl mb-7">
                    <Image
                      src={image}
                      alt={t(`${key}.title`)}
                      fill
                  ...
                  <p className="font-sans text-sm font-semibold text-sea tracking-wide2 mb-3">0{i + 1}</p>
                  <h2 className="font-sans text-3xl md:text-4xl font-light text-ink tracking-tight mb-4">
                    {t(`${key}.title`)}
                  </h2>
